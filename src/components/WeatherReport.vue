...
const fetchWeather = async (coords: Coords): Promise<WeatherData> => {
    ...
    const res = await fetch(
        `https://api.weatherapi.com/v1/current.json?key=${  import.meta.env.VUE_APP_API  }&q=${q}`
    );
...
<template>
    <div>
        <article v-if="data && data.current" class="max-w-md w-96 rounded-lg p-4 flex bg-indigo-500 text-white bg-opacity-25">
